...
def load_state_dict(model, path, exact=True):
    try:
...
def load_model(model, state_dict, exact=True):
    def filter(x):
        return x[7:] if x.startswith('module.') else x
    ...
    # if "encoder.embeddings.position_ids" not in state_dict.keys():
    #     state_dict["encoder.embeddings.position_ids"] = torch.arange(0, 514, device=None).view(1, -1).cuda()
    if "encoder_q.encoder.embeddings.position_ids" not in state_dict.keys():
        state_dict["encoder_q.encoder.embeddings.position_ids"] = torch.arange(0, 514, device=None).view(1, -1).cuda()
    if "encoder_k.encoder.embeddings.position_ids" not in state_dict.keys():
        state_dict["encoder_k.encoder.embeddings.position_ids"] = torch.arange(0, 514, device=None).view(1, -1).cuda()
...
# def move_to_ds_cuda(sample, device, data_type):
#     if len(sample) == 0:
#         return {}
#
...
#             if sample[k].dtype != torch.int64:
#                 kwargs.update(dict(dtype=data_type))
#
#             sample[k] = v.to(**kwargs)
#     return sample
