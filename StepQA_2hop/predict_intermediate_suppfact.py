        ...
        if not args.do_predict:
            ...
            for epoch in range(start_epoch, int(args.num_train_epochs)):
                ...
                for batch in tqdm(train_dataloader):
                    ...
                    sp_loss, sp_loss_2, span_loss = model(batch)
                    loss = (sp_loss * 2 + sp_loss_2) * args.sp_lambda + span_loss
                    ...
                    if (batch_step + 1) % args.gradient_accumulation_steps == 0:
                        ...
                        if batch_step % args.logging_steps == 0 and args.local_rank in [-1, 0]:
                            ...
                            logger.info("span loss: %s; sp loss: %s; sp loss 2: %s.",
                                        str(round(span_loss_meter.avg, 4)), str(round(sp_loss_meter.avg, 4)),
                                        str(round(sp_loss_2_meter.avg, 4)))
...
def predict(args, model, eval_dataloader, logger):
    ...
    second_predictions = (all_start_logits, all_end_logits, all_sp_scores_2)
    ...
    if args.do_predict:
        if args.local_rank in [-1, 0]:
            if "train" in args.predict_file:
                json.dump(first_sp_predictions, open("../Data/HotpotQA/processed/train_first_sp_predictions_large.json", "w"))
            else:
                json.dump(first_sp_predictions, open("../Data/HotpotQA/processed/dev_first_sp_predictions_large.json", "w"))
    ...
    for i, example_index in tqdm(enumerate(all_ids)):
        if example_index < len(examples):
            ...
            if is_first==1:
                ...
                sorted_title_score_list = sorted(title_score_list, key=lambda y: y[1], reverse=True)
                for n in range(min(2, len(title_score_list))):
                    for each in pred_sp_dict[sorted_title_score_list[n][0]]:
                        pred_sp.append([sorted_title_score_list[n][0], each[0]])
            ...
            if args.local_rank in [-1, 0]:
                if i < 5:
                    if is_first==1:
                        ...
                        print(pred_sp, "/", example["first_supporting_facts"]+example["second_supporting_facts"])
                        
            metrics = {'sp_em': 0, 'sp_f1': 0, 'sp_prec': 0, 'sp_recall': 0}
            if is_first==1:
                update_sp(metrics, pred_sp, example["first_supporting_facts"])
            else:
                update_sp(metrics, pred_sp, example["first_supporting_facts"]+example["second_supporting_facts"])
