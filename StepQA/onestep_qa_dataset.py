...
class OnestepQADataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        all_sent_label = [max(sample["sp_sent_first_labels"][i], sample["sp_sent_second_labels"][i], sample["sp_sent_third_labels"][i], sample["sp_sent_forth_labels"][i]) for i in range(len(sample["sp_sent_forth_labels"]))]
...
def qa_collate(samples, pad_id=0, neg_num=2):
    ...
    if "forth_sent_label" in samples[0]:
