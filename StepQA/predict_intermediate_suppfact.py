...
from intermediate_suppfact_dataset import IntSuppFactDataset, qa_collate
from intermediate_suppfact_model import IntSuppFactModel
...
def main():
    ...
    model = IntSuppFactModel(bert_config, args)
    ...
    eval_dataset = IntSuppFactDataset(tokenizer, args.predict_file, args.max_seq_len)
    ...
    if args.do_train:
        ...
        if args.init_checkpoint != "" and args.general_checkpoint:
            ...
            sp_loss_2_meter = AverageMeter()
            sp_loss_3_meter = AverageMeter()
        ...
        model.train()
        train_dataset = IntSuppFactDataset(tokenizer, args.train_file, args.max_seq_len, train=True)
        if args.deepspeed:
        ...
        if not args.do_predict:
            ...
            for epoch in range(start_epoch, int(args.num_train_epochs)):
                ...
                for batch in tqdm(train_dataloader):
                    ...
                    sp_loss, sp_loss_2, sp_loss_3, sp_loss_4, span_loss = model(batch)
                    loss = (sp_loss + sp_loss_2 + sp_loss_3) / 3 * args.sp_lambda + args.sp_lambda * sp_loss_4 + span_loss
                    ...
                    sp_loss_2_meter.update(sp_loss_2.item())
                    sp_loss_3_meter.update(sp_loss_3.item())
                    ...
                    if (batch_step + 1) % args.gradient_accumulation_steps == 0:
                        ...
                        if batch_step % args.logging_steps == 0 and args.local_rank in [-1, 0]:
                            ...
                            logger.info("span loss: %s; sp loss: %s; sp loss 2: %s; sp loss 3: %s; sp loss 4: %s.",
                                        str(round(span_loss_meter.avg, 4)), str(round(sp_loss_meter.avg, 4)),
                                        str(round(sp_loss_2_meter.avg, 4)), str(round(sp_loss_3_meter.avg, 4)),
                                        str(round(sp_loss_4_meter.avg, 4)))
    ...
    if args.do_predict:
        if args.local_rank in [-1, 0]:
            if "train" in args.predict_file:
                json.dump(first_sp_predictions, open("../Data/2WikiMultiHopQA/processed/train_first_sp_predictions_large.json", "w"))
                json.dump(second_sp_predictions, open("../Data/2WikiMultiHopQA/processed/train_second_sp_predictions_large.json", "w"))
                json.dump(third_sp_predictions, open("../Data/2WikiMultiHopQA/processed/train_third_sp_predictions_large.json", "w"))
            else:
                json.dump(first_sp_predictions, open("../Data/2WikiMultiHopQA/processed/dev_first_sp_predictions_large.json", "w"))
                json.dump(second_sp_predictions, open("../Data/2WikiMultiHopQA/processed/dev_second_sp_predictions_large.json", "w"))
                json.dump(third_sp_predictions, open("../Data/2WikiMultiHopQA/processed/dev_third_sp_predictions_large.json", "w"))
