...
from step_qa_dataset import StepQADataset, qa_collate
...
def main():
    ...
    eval_dataset = StepQADataset(tokenizer, args.predict_file, args.max_seq_len, ques_ans_file_dir=args.ques_ans_file_dir)
    ...
    if args.do_train:
        ...
        if args.init_checkpoint != "" and args.general_checkpoint:
            ...
            sp_loss_4_meter = AverageMeter()
            end_loss_meter = AverageMeter()
        else:
        ...
        model.train()
        train_dataset = StepQADataset(tokenizer, args.train_file, args.max_seq_len, train=True, ques_ans_file_dir=args.ques_ans_file_dir)
        if args.deepspeed:
        ...
        if not args.do_predict:
            ...
            for epoch in range(start_epoch, int(args.num_train_epochs)):
                ...
                for batch in tqdm(train_dataloader):
                    ...
                    inter_sp_loss, sp_loss_4, span_loss, end_loss = model(batch)
                    loss = inter_sp_loss * args.sp_lambda + args.sp_lambda * sp_loss_4 + span_loss + end_loss * 2
                    ...
                    sp_loss_meter.update(inter_sp_loss.item())
                    sp_loss_4_meter.update(sp_loss_4.item())
                    end_loss_meter.update(end_loss.item())
                    ...
                    if (batch_step + 1) % args.gradient_accumulation_steps == 0:
                        ...
                        if batch_step % args.logging_steps == 0 and args.local_rank in [-1, 0]:
                            ...
                            logger.info("span loss: %s; inter sp loss: %s; sp loss 4: %s; end loss: %s.",
                                        str(round(span_loss_meter.avg, 4)), str(round(sp_loss_meter.avg, 4)),
                                        str(round(sp_loss_4_meter.avg, 4)), str(round(end_loss_meter.avg, 4)))
...
def predict(args, model, eval_dataloader, logger):
    ...
    all_index = None
    sp_scores_host = None
    sp_scores_host_2 = None
    sp_scores_host_3 = None
    sp_scores_host_4 = None

    all_sp_scores = None
    all_sp_scores_2 = None
    all_sp_scores_3 = None
    all_sp_scores_4 = None

    end_scores_host_1 = None
    end_scores_host_2 = None
    end_scores_host_3 = None

    all_end_scores_1 = None
    all_end_scores_2 = None
    all_end_scores_3 = None
    ...
    for batch in tqdm(eval_dataloader):
        ...
        with torch.no_grad():
            ...
            sp_scores_2 = outputs["sp_scores_2"].sigmoid()
            sp_scores_3 = outputs["sp_scores_3"].sigmoid()
            sp_scores_4 = outputs["sp_scores_4"].sigmoid()

            end_scores_1 = outputs["end_score_1"].sigmoid()
            end_scores_2 = outputs["end_score_2"].sigmoid()
            end_scores_3 = outputs["end_score_3"].sigmoid()
        ...
        sp_scores_2 = _pad_across_processes(sp_scores_2, args)
        sp_scores_2 = _nested_gather(sp_scores_2, args)
        sp_scores_host_2 = sp_scores_2 if sp_scores_host_2 is None else nested_concat(sp_scores_host_2, sp_scores_2,
                                                                                padding_index=-100)

        sp_scores_3 = _pad_across_processes(sp_scores_3, args)
        sp_scores_3 = _nested_gather(sp_scores_3, args)
        sp_scores_host_3 = sp_scores_3 if sp_scores_host_3 is None else nested_concat(sp_scores_host_3, sp_scores_3,
                                                                                padding_index=-100)

        sp_scores_4 = _pad_across_processes(sp_scores_4, args)
        sp_scores_4 = _nested_gather(sp_scores_4, args)
        sp_scores_host_4 = sp_scores_4 if sp_scores_host_4 is None else nested_concat(sp_scores_host_4, sp_scores_4,
                                                                                padding_index=-100)
        ...
        end_scores_1 = _pad_across_processes(end_scores_1, args)
        end_scores_1 = _nested_gather(end_scores_1, args)
        end_scores_host_1 = end_scores_1 if end_scores_host_1 is None else nested_concat(end_scores_host_1, end_scores_1, padding_index=-100)

        end_scores_2 = _pad_across_processes(end_scores_2, args)
        end_scores_2 = _nested_gather(end_scores_2, args)
        end_scores_host_2 = end_scores_2 if end_scores_host_2 is None else nested_concat(end_scores_host_2, end_scores_2, padding_index=-100)

        end_scores_3 = _pad_across_processes(end_scores_3, args)
        end_scores_3 = _nested_gather(end_scores_3, args)
        end_scores_host_3 = end_scores_3 if end_scores_host_3 is None else nested_concat(end_scores_host_3, end_scores_3, padding_index=-100)
    ...
    if sp_scores_host_2 is not None:
        _sp_scores_2 = nested_numpify(sp_scores_host_2)
        all_sp_scores_2 = _sp_scores_2 if all_sp_scores_2 is None else nested_concat(all_sp_scores_2, _sp_scores_2, padding_index=-100)

    if sp_scores_host_3 is not None:
        _sp_scores_3 = nested_numpify(sp_scores_host_3)
        all_sp_scores_3 = _sp_scores_3 if all_sp_scores_3 is None else nested_concat(all_sp_scores_3, _sp_scores_3, padding_index=-100)

    if sp_scores_host_4 is not None:
        _sp_scores_4 = nested_numpify(sp_scores_host_4)
        all_sp_scores_4 = _sp_scores_4 if all_sp_scores_4 is None else nested_concat(all_sp_scores_4, _sp_scores_4, padding_index=-100)
    ...
    if end_scores_host_1 is not None:
        _end_scores_1 = nested_numpify(end_scores_host_1)
        all_end_scores_1 = _end_scores_1 if all_end_scores_1 is None else nested_concat(all_end_scores_1, _end_scores_1, padding_index=-100)

    if end_scores_host_2 is not None:
        _end_scores_2 = nested_numpify(end_scores_host_2)
        all_end_scores_2 = _end_scores_2 if all_end_scores_2 is None else nested_concat(all_end_scores_2, _end_scores_2, padding_index=-100)

    if end_scores_host_3 is not None:
        _end_scores_3 = nested_numpify(end_scores_host_3)
        all_end_scores_3 = _end_scores_3 if all_end_scores_3 is None else nested_concat(all_end_scores_3, _end_scores_3, padding_index=-100)

    all_start_logits = np.array(all_start_logits)
    all_end_logits = np.array(all_end_logits)

    all_sp_scores = np.array(all_sp_scores)
    all_sp_scores_2 = np.array(all_sp_scores_2)
    all_sp_scores_3 = np.array(all_sp_scores_3)
    all_sp_scores_4 = np.array(all_sp_scores_4)

    all_index = np.array(all_index)
    all_ids = all_index.reshape(-1).tolist()

    all_end_scores_1 = np.array(all_end_scores_1)
    all_end_scores_2 = np.array(all_end_scores_2)
    all_end_scores_3 = np.array(all_end_scores_3)
    all_end_predictions = (all_end_scores_1, all_end_scores_2, all_end_scores_3)

    postprocess_end_predictions(examples=eval_dataloader.dataset.data, all_ids=all_ids, end_predictions=all_end_predictions)

    first_predictions = (None, None, all_sp_scores)
    _, first_sp_predictions, first_eval_metrics_all = postprocess_qa_predictions(
        args,
    ...
    second_predictions = (None, None, all_sp_scores_2)
    _, second_sp_predictions, second_eval_metrics_all = postprocess_qa_predictions(
        args,
        examples=eval_dataloader.dataset.data,
        features=eval_dataloader.dataset.features,
        all_ids=all_ids,
    ...
    third_predictions = (None, None, all_sp_scores_3)
    _, third_sp_predictions, third_eval_metrics_all = postprocess_qa_predictions(
        args,
        examples=eval_dataloader.dataset.data,
        features=eval_dataloader.dataset.features,
        all_ids=all_ids,
        predictions=third_predictions,
        n_best_size=20,
        max_answer_length=args.max_ans_len,
        is_first=3,
    )

    forth_predictions = (all_start_logits, all_end_logits, all_sp_scores_4)
    ...
    # span_references = [{"id": sample["_id"], "answers": sample["answer"]} for sample in
    ...
    if args.local_rank in [-1, 0]:
        ...
        logger.info(f"First Metrics: {first_eval_metrics_all}")
        logger.info(f"Second Metrics: {second_eval_metrics_all}")
        logger.info(f"Third Metrics: {third_eval_metrics_all}")
        logger.info(f"Last Metrics: {forth_eval_metrics_all}")
    model.train()
    ...
    for k, v in first_eval_metrics_all.items():
        metrics[f"first_{k}"] = v

    for k, v in second_eval_metrics_all.items():
        metrics[f"second_{k}"] = v

    for k, v in third_eval_metrics_all.items():
        metrics[f"third_{k}"] = v
...
def postprocess_end_predictions(
    examples,
    all_ids,
    end_predictions
):
    first_end_scores, second_end_scores, third_end_scores = end_predictions
    accuracy_list = list()
    for i, example_index in tqdm(enumerate(all_ids)):
        if example_index < len(examples):
            example = examples[example_index]

            if example['end_label'][1] == 1:
                if first_end_scores[i] >= 0.5:
                    accuracy_list.append(1)
                else:
                    accuracy_list.append(0)
            else:
                if first_end_scores[i] < 0.5 and second_end_scores[i] < 0.5 and third_end_scores[i] >= 0.5:
                    accuracy_list.append(1)
                else:
                    accuracy_list.append(0)
        
    print("End prediction accuracy", np.mean(accuracy_list))
