...
class IntSuppFactDataset(Dataset):
    ...
    def __init__(self,
        ...
        if not train:
            ...
            for i, sample in tqdm(enumerate(self.data)):
                ...
                q_sp_codes = self.tokenizer.encode_plus("HOP=4 [SEP] " + question, text_pair=sample["selected_context"].strip(),
    ...
    def __getitem__(self, index):
        ...
        # third sp data
        q_sp_codes_3 = self.tokenizer.encode_plus("HOP=3 [SEP] " + question, text_pair=context.strip(),
                                                max_length=self.max_seq_len, return_tensors="pt",
                                                truncation='longest_first', return_offsets_mapping=True)
        ...
        q_sp_codes_4 = self.tokenizer.encode_plus("HOP=4 [SEP] " + question, text_pair=context.strip(),
                                                  max_length=self.max_seq_len, return_tensors="pt",
                                                  truncation='longest_first', return_offsets_mapping=True)
